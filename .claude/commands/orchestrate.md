...
8. **Do not call `ScheduleWakeup` yourself from within this command.** This command runs as the "run the parsed prompt now" step of the `/loop` skill's dynamic-mode contract — that outer `/loop` invocation is the one that calls `ScheduleWakeup` afterward (with `prompt: "/loop /orchestrate"`, prefixed, per its own step 4/5). Calling it here too would double-schedule or use the wrong prompt format. Just make the run_status/TASKS.md state correct — the wrapper reads it back next firing.
9. End with **exactly one** status line: `ITERATION COMPLETE: TASK-NNN <PASS|FAIL>, X/Y done` or `RUN STOPPED: <reason>`. This is what the outer `/loop` wrapper uses to decide whether to keep going or call `ScheduleWakeup(stop: true)`. Keep everything else minimal — this runs unattended overnight, there's no one reading verbose narration.
